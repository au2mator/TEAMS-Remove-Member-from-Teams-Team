...
# New Service
# M365 - Remove Member from Teams Team
# v 1.0 Initial Release
# Init Release: 18.05.2020
# Last Update: 18.05.2020
# Code Template V 1.1
# URL: https://au2mator.com/remove-member-from-microsoft-teams-team–microsoft-teams-self-service-with-au2mator/
...
try{
    ...
    if ($Team)
    {
        ...
        try{
            ...
            $SplitMembers=$c_TeamMembers.split(";")

                foreach ($Member in $SplitMembers) {
                    Write-au2matorLog -Type INFO -Text "Remove Member: $Member"
                    $Team | Remove-TeamUser -User $Member
...
if ($ErrorCount -eq 0) {
    $au2matorReturn = "Team Member has been removed"
    $AdditionalHTML = "
        "
    $Status = "COMPLETED"
}
else {
    $au2matorReturn = "failed to remove Team Member, Error: $Error"
