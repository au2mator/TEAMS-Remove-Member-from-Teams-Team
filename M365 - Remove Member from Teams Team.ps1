...
# Type: New Service
#
# Title: M365 - Remove Member from Teams Team
#
# v 1.0 Initial Release
# v 1.1 Added Stored Credentials
#       see for details: https://au2mator.com/documentation/powershell-credentials/?utm_source=github&utm_medium=social&utm_campaign=M365_RemoveMemberFromTeams&utm_content=PS1
...
# Code Template V 1.3
#
# URL: https://au2mator.com/remove-member-from-microsoft-teams-team–microsoft-teams-self-service-with-au2mator//?utm_source=github&utm_medium=social&utm_campaign=M365_RemoveMemberFromTeams&utm_content=PS1
# Github: https://github.com/au2mator/TEAMS-Remove-Member-from-Teams-Team
#################
...
[string]$LogPath = "C:\_SCOworkingDir\TFS\PS-Services\M365 - Remove Member from Teams Team"
[string]$LogfileName = "Remove Member from Teams Team"

[string]$CredentialStorePath = "C:\_SCOworkingDir\TFS\PS-Services\CredentialStore" #see for details: https://au2mator.com/documentation/powershell-credentials/?utm_source=github&utm_medium=social&utm_campaign=M365_RemoveMemberFromTeams&utm_content=PS1
...
# Stored Credentials
# See: https://au2mator.com/documentation/powershell-credentials/?utm_source=github&utm_medium=social&utm_campaign=M365_RemoveMemberFromTeams&utm_content=PS1
...
try {
    ...
    try {
        Write-au2matorLog -Type INFO -Text "Try to get the Team with ID"

        $URL = "https://graph.microsoft.com/v1.0/groups"  
        $Team = (Invoke-RestMethod -Headers $headers -Uri $URL -Method GET).value | Where-Object -Property ID -Value $c_TeamID -eq
        
        Write-au2matorLog -Type INFO -Text "Got the Team with Name: $($Team.displayname)"

        try {
            Write-au2matorLog -Type INFO -Text "Try to get User with UPN"
            $TempMember = $c_TeamMembers
            $URL = "https://graph.microsoft.com/v1.0/groups/$c_TeamID/members"  
            $Result = Invoke-RestMethod -Headers $headers -Uri $URL -Method GET -Verbose
            $UserID = ($Result.value | Where-Object -Property userPrincipalName -Value "$TempMember" -eq).ID
            Write-au2matorLog -Type INFO -Text "Got the User ID: $UserID"

            try {
                Write-au2matorLog -Type INFO -Text "Try to remove the User from the Team"

               
                $URL = "https://graph.microsoft.com/v1.0/groups/$c_TeamID/members/$UserID/`$ref"  
                Invoke-RestMethod -Headers $headers -Uri $URL -Method DELETE -Verbose
    
            

                $au2matorReturn = "The User $TempMember was removed from to Teams $($Team.displayname)"
                $AdditionalHTML = "The User $TempMember was removed from Teams $($Team.displayname)
            <br>
            "
                $Status = "COMPLETED"
            }
                ...
                Write-au2matorLog -Type ERROR -Text "Could not remove Member"
                Write-au2matorLog -Type ERROR -Text "Error: $Error"

                $au2matorReturn = "Could not remove Member, Error: $Error"
                $AdditionalHTML = "Could not remove Member
            <br>
            Error: $Error
            "
                $Status = "ERROR"
            ...
            Write-au2matorLog -Type ERROR -Text "Could not get ID for the User"
            Write-au2matorLog -Type ERROR -Text "Error: $Error"

            $au2matorReturn = "Could not get ID for the User, Error: $Error"
            $AdditionalHTML = "Could not get ID for the User
        <br>
        Error: $Error
        "
            $Status = "ERROR"
        ...
        Write-au2matorLog -Type ERROR -Text "Could not Query Team with ID"
        Write-au2matorLog -Type ERROR -Text "Error: $Error"

        $au2matorReturn = "Could not Query Team with ID $c_TeamID, Error: $Error"
        $AdditionalHTML = "Could not Query Team with ID $c_TeamID
        <br>
        Error: $Error
            "
        $Status = "ERROR"
